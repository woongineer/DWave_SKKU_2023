...
def visualize(G, bqm, sampleset, problem_filename, solution_filename):
    ...
    edgecolors = [
        'g' if bqm.get_quadratic(a, b) < 0 else 'r' if bqm.get_quadratic(a,
                                                                         b) > 0 else 'b'
        for a, b in
        G.edges()]
    ...
    nx.draw_networkx(G, pos=pos, edge_color=edgecolors, with_labels=True,
                     node_color='k', font_color='w')
...
def process_sampleset(G, sampleset):
    ...
    print('{:>15s}{:>15s}{:^15s}'.format('0', str(set0_friendly),
                                         str(set0_hostile)))
    print('{:>15s}{:>15s}{:^15s}'.format('1', str(set1_friendly),
                                         str(set1_hostile)))
    print('{:>15s}{:>15s}{:^15s}'.format('0 -> 1', str(cut_friendly),
                                         str(cut_hostile)))
...
if __name__ == "__main__":
    ...
    if sampleset.variables != []:
        # Visualize results
        if viz:
            visualize(G, bqm, sampleset, "hybrid_problem_graph.png",
                      "hybrid_solution_graph.png")
