...
def get_qubo(S):
    ...
    for i in range(len(S)):
        for j in range(len(S)):
            if i < j:  #대각선 아래쪽은 0이니까 위에만
                Q[(i, j)] = 8 * S[i] * S[j]

    for i in range(len(S)):
        Q[(i, i)] = -4 * sum(S) * S[i] + 4 * S[i] * S[i]
...
def run_on_qpu(Q, sampler):
    ...
    chainstrength = 100000000  # update, 이게 중요한 듯. 100정도 해도 개판인음
    numruns = 100  # update

    sample_set = sampler.sample_qubo(Q, chain_strength=chainstrength,
                                     num_reads=numruns,
                                     label='Training - Number Partitioning')
